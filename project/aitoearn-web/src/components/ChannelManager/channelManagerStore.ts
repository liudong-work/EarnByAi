...
function pickFirstValidString(...values: unknown[]): string | null {
...
function normalizeAuthUrlResponse(res: any): AuthUrlResponse {
  if (!res) {
    throw new Error('Network error')
  }

  if ('code' in res && Number(res.code) !== 0) {
    if (Number(res.code) === 401) {
      useUserStore.getState().logout()
    }
    throw new Error(res.message || 'Failed to get auth URL')
  }

  const data = typeof res.data === 'object' && res.data ? res.data : {}
  const url = pickFirstValidString(data.url, data.uri, res.url)
  const taskId = pickFirstValidString(
    data.taskId,
    data.id,
    data.state,
    res.taskId,
  ...
  if (!taskId) {
    throw new Error(res.message || 'Missing auth task ID')
  }
...
async function getAuthUrl(platform: PlatType, spaceId?: string): Promise<AuthUrlResponse | null> {
  ...
  switch (platform) {
    case PlatType.KWAI:
      ...
      return normalizeAuthUrlResponse(res)
